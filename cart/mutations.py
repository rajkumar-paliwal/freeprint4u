...
class UpdateCartMutation(graphene.Mutation):
    ...
    ok = graphene.Boolean()
    cart_owner = graphene.Field(CartType)

    def mutate(self, info,id=None,userid=None):
        id = from_global_id(id)[1]
        userObj = User.objects.get(id=from_global_id(userid)[1])
        trackingObj = Cart.objects.filter(id=id).update(userid=userObj)
        ok = True
        return UpdateCartMutation(cart_owner=Cart.objects.get(id=id), ok=ok)
